(function () {
    ...
    function getDomain() {
        let domain = window.location.origin,
            regr1 = /https?:\/\/\w*\.(\w+\.\w+)/i;
        regr2 = /https?:\/\/(\w+\.\w+)/i;
        if (regr1.test(domain)) {
            domain = regr1.exec(domain)[1];
            return domain;
        } else {
            domain = regr2.exec(domain)[1];
            return domain;
        }
    }
    ...
    chrome.runtime.onMessage.addListener(
        function (request, sender, sendResponse) {
            if (request.needLink == true) {
                ...
                chrome.runtime.sendMessage({ link: [{ domain, aText, href: link }], isLink: true }, (response) => {
                    if (response.success == true) {
                ...
                let selectedLink = document.getElementsByClassName("selectiveLinks"),
                    domain = getDomain(), data = [];
                for (let i = 0; i < selectedLink.length; i++) {
                    let element = selectedLink[i];
                    data.push({ href: element.href, aText: element.innerText, domain });
                }
